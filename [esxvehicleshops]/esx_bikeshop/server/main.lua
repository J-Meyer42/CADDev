...
TriggerEvent('esx_society:registerSociety', 'bike', 'Concessionnaire', 'society_dock', 'society_dock', 'society_dock', {type = 'private'})
...
AddEventHandler('esx_bikeshop:setVehicleOwned', function (vehicleProps, vModel)
...
RegisterServerEvent('esx_bikeshop:setVehicleForAllPlayers')
AddEventHandler('esx_bikeshop:setVehicleForAllPlayers', function (props, x, y, z, radius)
  TriggerClientEvent('esx_bikeshop:setVehicle', -1, props, x, y, z, radius)
end)
...
ESX.RegisterServerCallback('esx_bikeshop:getPersonnalVehicles', function (source, cb)
  local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_bikeshop:resellVehicle', function (source, cb, plate, price)
  MySQL.Async.fetchAll(
    'SELECT * FROM rented_vehicles WHERE plate = @plate LIMIT 1',
    { ['@plate'] = plate },
    function (result)
...
if Config.EnablePvCommand then
  TriggerEvent('es:addCommand', 'bt', function (source, args, user)
    TriggerClientEvent('esx_bikeshop:openPersonnalVehicleMenu', source)
  end, {help = _U('leaving')})
end
