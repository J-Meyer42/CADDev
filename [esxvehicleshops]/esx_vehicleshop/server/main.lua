...
AddEventHandler('esx_vehicleshop:setVehicleOwned', function(vehicleProps, vModel)
	...
	TriggerEvent('menu:getCharID', xPlayer.identifier, function(callback, data)
	...
	MySQL.Async.execute(
		'INSERT INTO owned_vehicles (vehicle, owner, charID) VALUES (@vehicle, @owner, @charID)',
		{
...
AddEventHandler('esx_vehicleshop:rentVehicle', function(vehicle, plate, playerName, basePrice, rentPrice, target)
	...
	MySQL.Async.fetchAll(
		...
		function(result)
			...
			MySQL.Async.execute(
				'INSERT INTO rented_vehicles (vehicle, plate, player_name, base_price, rent_price, owner, charID) VALUES (@vehicle, @plate, @player_name, @base_price, @rent_price, @owner, @charID)',
				{
					['@vehicle']     = vehicle,
					['@plate']       = plate,
...
ESX.RegisterServerCallback('esx_vehicleshop:getPersonnalVehicles', function(source, cb)
	...
	MySQL.Async.fetchAll(
		'SELECT * FROM owned_vehicles WHERE owner = @owner AND charID = @charID',
		{
			['@owner'] = xPlayer.identifier,
			['@charID'] = charID
		},
