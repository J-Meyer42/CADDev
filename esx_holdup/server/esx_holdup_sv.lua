...
AddEventHandler('esx_holdup:rob', function(robb, loc)
	...
	if Stores[robb] then
		...
		if (os.time() - lastRobberyTime) < 600 and lastRobberyTime ~= 0 then
			TriggerClientEvent('esx:showNotification', source, _U('already_robbed') .. (1800 - (os.time() - lastRobberyTime)) .. _U('seconds'))
			return
		end

		if (os.time() - store.lastrobbed) < 1200 and store.lastrobbed ~= 0 then
		...
		if rob == false then
			...
			local robberyTimer = math.random(45000,75000)
			...
			if(cops > 2)then
				...
				TriggerClientEvent('esx_holdup:currentlyrobbing', source, robb, (math.floor(robberyTimer / 1000)))
				Stores[robb].lastrobbed = os.time()
				lastRobberyTime = os.time()
				robbers[source] = robb
				local savedSource = source
				SetTimeout(robberyTimer, function()
