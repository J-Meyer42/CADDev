...
AddEventHandler('esx_phone:billCall', function(duration)
  ...
  local _source = source
  local xPlayer = ESX.GetPlayerFromId(_source)
  local amount  = math.floor(duration * 0.0)

  xPlayer.removeAccountMoney('bank', amount)

  TriggerClientEvent('esx:showNotification', _source, 'Your call will cost you ~r~$' .. amount .. '~s~')
...
AddEventHandler('esx_phone:bankTransfer', function(target, amount)
  ...
  if amount > 0 and amount <= xPlayer.getAccount('bank').money then
    ...
    TriggerClientEvent('esx:showNotification', _source, 'You have been charged ~g~$' .. amount .. '~s~ from ' .. targetXPlayer.name)
